...
sparkle watermark. The cleaned file lands in `Downloads/mark-killer/`. Nothing is
...
toolbar panel and choose *Clean a video*. The controls live in the panel itself.

The export does not. A toolbar panel is destroyed the moment it loses focus, and
an export runs for minutes, so the work is handed to an **offscreen document** —
a page with no window that keeps running after the panel closes. Close the
panel, switch tabs, keep working: the cleaned video still saves itself. Reopen
the panel and it picks the progress back up.

That split shapes the code. An offscreen document is given `chrome.runtime` and
nothing else — no `chrome.storage`, no `chrome.downloads` — so the video is
passed to it through IndexedDB (both live on the same extension origin) and the
service worker owns the job state and the saving. A standalone window
(`src/studio.html`) is kept as a fallback for Chrome versions that will not open
the panel on the extension's behalf.
...
src/
  ...
  video-view.js/.css   the video controls, mounted in the panel and the window
  studio.html/.css/.js the standalone window, used when the panel cannot open
  offscreen.html/.js    runs the export where closing the panel cannot stop it
  engine/
  ...
  lib/jobstore.js      passing the video between panel, worker and offscreen
...
| `storage` | remembering the hover-button setting and the state of a running export |
| `offscreen` | keeping an export alive after the panel closes |
...
* `tools/e2e.mjs` — loads the extension into a real headless browser, runs the
  ...
  to disk, builds a watermarked H.264 clip and runs the whole video pipeline over
  it, and finally drives a real export through the offscreen document and checks
  the file it saves on its own.
...
npm run preview:video    # the video panel    -> video-preview.png
```
